 ...
 * @author TTC
 */
@WebServlet(name = "CreateDM", urlPatterns = {"/admin/createDM"})
public class CreateDM extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (dmDao.details(madm) != null) {
            request.setAttribute("errorMadm", "Mã danh mục đã tồn tại");
            back = true;
        }
        if (dmDao.checkMadm(madm) != "") {
            request.setAttribute("errorMadm", dmDao.checkMadm(madm));
            back = true;
        }
        if (tendm.length() < 5) {
        ...
        if (back) {
            ...
            DanhMuc dm = new DanhMuc(madm, tendm, true);
            dmDao.create(dm);
