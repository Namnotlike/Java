 ...
 * @author TTC
 */
@WebServlet(name = "CreateSP", urlPatterns = {"/admin/createSP"})
public class CreateSP extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (spDao.details(masp) != null) {
            request.setAttribute("errorMasp", "Mã sản phẩm đã tồn tại");
            back = true;
        }
        if (spDao.checkMasp(masp) != "") {
            request.setAttribute("errorMasp", spDao.checkMasp(masp));
            back = true;
        }
        if (tensp.length() < 5) {
        ...
        if (!back) {
            SanPham sp = new SanPham(masp, tensp, mota, soluong, dongia, hinhanh, true, dmDao.details(madm), nccDao.details(mancc));
            spDao.create(sp);
